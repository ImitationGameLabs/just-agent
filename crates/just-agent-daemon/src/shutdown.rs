//! Daemon-wide agent shutdown: bounded graceful drain with a force-abort safety net.
//!
//! Daemon exit previously blocked for a fixed timeout (`tokio::time::sleep`)
...
///
/// Called after the HTTP server has stopped and the daemon-wide `shutdown` token
