...
mod tests {
    ...
    async fn approval_policy_gate_allows_when_superior_has_allow() {
        ...
        let result = crate::routes::approval::respond_approval(
            AxumState(state),
            crate::auth::AuthIdentity::test_new(Identity::Agent { id: parent }),
    ...
    async fn approval_policy_gate_operator_exempt() {
        ...
        let result = crate::routes::approval::respond_approval(
            AxumState(state),
            crate::auth::AuthIdentity::test_new(Identity::Operator),
            AxumPath(approval_id),
            Json(ApprovalDecisionBody {
                decision: "approve".into(),
                reason: None,
    ...
    async fn approval_deny_always_allowed_regardless_of_policy() {
        ...
        {
            let mut reg = state.registry.write().await;
            // Parent has Deny for the tool — still allowed to deny the approval.
            add_root_with_policy(
                &mut reg,
                &parent,
                policy_for_tool("dangerous_tool", PolicyDecision::Deny),
    ...
    async fn approval_policy_gate_checks_specific_tool_not_any_allow() {
        ...
        // Parent has Allow for safe_tool but Ask for dangerous_tool.
        ...
        // Approval requests dangerous_tool (Ask), not safe_tool (Allow).
        let approval_id =
            enqueue_committed_approval(&state.registry.read().await, &child, "dangerous_tool")
                .await;
