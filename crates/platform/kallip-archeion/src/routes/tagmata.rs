...
use kallip_archeion_common::proof::{ProofError, verify_enroll_proof};
...
#[derive(Serialize)]
struct MintResponse {
    /// `sk-enroll-...` plaintext; returned once. The archeion retains only its
...
mod tests {
    ...
    async fn cap_counts_expired_pending_tagmas() {
        ...
        for n in 0..MAX_LIVE_PENDING_TAGMAS {
            ...
            tagmata::ActiveModel {
                id: Set(kallip_archeion_common::ids::TagmaId::random().to_string()),
