...
/// (`admin`, in `kallip-archeion-common`), NOT the subject: the marker pair is
/// what identifies the account, while the name doubles as a reserved
/// signup handle. The constants live in `kallip-archeion-common` and are
...
/// `KALLIP_ARCHEION_ADMIN_USER_LOGIN` is set (see `routes::router`); the route
