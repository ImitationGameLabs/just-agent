...
//! see state shaped exactly as a live archeion would produce it.
//!
...
pub(crate) async fn setup_test_db() -> Db {
    ...
    let db_name = format!("archeion_test_{}_{n}", std::process::id());
...
// the archeion was a monolith. They moved to the `kallip-lesche` test harness
