...
//! but resolves to archeion principals (admin / user / tagma) rather than the
...
// `kallip-archeion-common` crate so both the registry and the relay enforce the
// same deputy guard. Re-export them here so existing `crate::auth::Principal` /
// `super::require_*` references keep resolving.
pub use kallip_archeion_common::principal::{Principal, require_admin, require_user};
...
impl FromRequestParts<SharedState> for AuthPrincipal {
    ...
    async fn from_request_parts(
        ...
        if let Some(cookie_value) = read_session_cookie(&parts.headers) {
            ...
            // The archeion's own routes need only the user id; the verified
...
// `kallip-archeion-common::principal` and are re-exported above. The disabled-user
