//! Identifier newtypes for the archeion subsystem.
//!
...
id_type! {
    /// Distributed-trace identifier propagated on envelopes. The archeion passes it
...
/// Pinned so the archeion (producer of `TagmaView.conversation_id`) and any client
...
impl ConversationId {
    ...
    /// it is stable across reconnects and archeion restarts and requires no
...
impl ParticipantId {
    ...
    /// archeion restarts; requires no storage.
