...
fn asks_for_cargo_test() {
    assert!(matches!(classify_command("cargo test"), ToolDecision::Ask));
}
...
#[test]
fn asks_for_heredoc() {
    assert!(matches!(classify_command("cat <<EOF"), ToolDecision::Ask));
}
...
fn asks_for_git_push() {
    assert!(matches!(classify_command("git push"), ToolDecision::Ask));
}
...
#[test]
fn asks_for_xargs() {
    assert!(matches!(classify_command("xargs rm"), ToolDecision::Ask));
}
...
#[test]
fn asks_for_umount() {
    assert!(matches!(classify_command("umount /mnt"), ToolDecision::Ask));
}
