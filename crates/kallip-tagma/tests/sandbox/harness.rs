...
/// Fixed operator token so we never need to parse the tagma banner.
...
/// How long to wait for the tagma to accept connections before giving up.
...
/// time from the tagma crate's manifest dir). The container overrides this
...
/// interpolate absolute paths (the agent's `$PWD` is the tagma process's cwd,
...
// ===========================================================================
// Tagma + runner subprocess control
// ===========================================================================

/// Owned tagma subprocess plus the captured stdout/stderr buffers (for
/// diagnostics on failure). `kill_on_drop` ensures the child dies if a guard
/// ever escapes explicit shutdown.
pub struct TagmaProc {
    child: Child,
    port: u16,
    url: String,
...
/// Bind an ephemeral port, read it, drop the listener, and hand it to the tagma
/// (the tagma never echoes its bound port, so `127.0.0.1:0` is unusable for
/// discovery). The bind->drop->rebind window is tiny; on the rare collision the
/// tagma fails to bind and the readiness check surfaces its captured output.
fn alloc_port() -> u16 {
...
/// Spawn the tagma against the mocked LLM endpoints and wait for it to accept
/// connections. Panics with the tagma's captured output if it fails to come up.
async fn spawn_tagma(world: &World, permission_class: Option<&str>) -> TagmaProc {
    let port = alloc_port();
    ...
    let mut env: Vec<(&str, String)> = vec![
        ...
        (
            "RUST_LOG",
            // Default to `info` (the tagma's own default) so startup errors
    ...
    let mut cmd = Command::new(resolve_bin("kallip-tagma"));
    cmd.args([
        "--listen-addr",
    ...
    let mut child = cmd.spawn().expect("spawn kallip-tagma");
    ...
    if !wait_ready(proc.port, READY_TIMEOUT).await {
        panic!("tagma did not become ready:\n{}", proc.diagnostics());
    }
    proc
}

/// Poll a TCP connect until the tagma is accepting connections, then settle
...
/// post the scenario prompt to the tagma's root agent. Captures its single JSON
/// stdout line.
pub async fn run_agent(tagma: &TagmaProc) -> RunResult {
    let output = Command::new(resolve_bin("kallip-run"))
        .args(["--prompt", "run the scripted sandbox checks", "--json"])
        .env_clear()
        .envs([
            ("KALLIP_TAGMA_URL", tagma.url.as_str()),
...
/// in declaration order, so `tagma` (with `kill_on_drop`) drops *before* the
/// `MockServer`s and `world`'s `TempDir`s. The two `MockServer`s are held here
/// -- not dropped at the end of `start` -- so they outlive the tagma's in-flight
...
pub struct Fixture {
    ...
    // the tagma's in-flight LLM calls (their Drop reclaims it), never read.
...
pub async fn start(
    ...
    let tagma = spawn_tagma(&world, permission_class).await;
    Fixture {
        tagma,
